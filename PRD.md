...
- 若调用大模型接口失败、超时（前端60秒、后端58秒超时）或返回为空，前端自动降级为原有的关键词规则问答（`buildAssistantReply`），保证助手在没有网络/接口异常时依然可用；
...
| 响应时延 | 大模型为推理模型，单次回复通常需要10~40秒，前端展示"正在思考"打字动效并禁用重复发送，前端60秒/后端58秒超时、Vercel 函数 `maxDuration` 设为60秒 |
| 消息格式 | 助手回复展示在小尺寸聊天气泡中，系统提示词限制模型只使用换行、列表和 **加粗** 三种格式，前端对 **加粗** 语法做轻量渲染，禁止标题/表格/代码块等复杂 Markdown |
